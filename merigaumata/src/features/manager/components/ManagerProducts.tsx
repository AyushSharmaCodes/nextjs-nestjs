...
export function ManagerProducts({
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold text-foreground tracking-tight">{t('products.vedicProducts')}</h2>
        <p className="text-sm text-neutral-500 dark:text-neutral-400 mt-1">{t('products.vedicProductsDesc')}</p>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 pt-2">
        <form onSubmit={handleAddProduct} className="xl:col-span-5 space-y-4 p-5 rounded-xl border border-border bg-neutral-50/50 dark:bg-neutral-900/40 h-max">
          <h3 className="font-bold text-xs uppercase tracking-wider text-neutral-400 dark:text-neutral-500 mb-1 flex items-center gap-2">
            <AppIcon name="plusCircle" className="w-4 h-4 text-primary-500" /> {t('products.createListing')}
          </h3>
          
          <div className="space-y-1">
            <label className="text-xs font-bold text-neutral-700 dark:text-neutral-300">{t('products.productTitle')}</label>
            <input 
              type="text" 
              required 
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              <label className="text-xs font-bold text-neutral-700 dark:text-neutral-300">{t('products.basePrice')}</label>
              <input 
                type="text" 
                required 
            ...
            <div className="space-y-1">
              <label className="text-xs font-bold text-neutral-700 dark:text-neutral-300">{t('products.initialStock')}</label>
              <input 
                type="number" 
                required 
                placeholder={t('products.placeholderStock')}
        ...
        <div className="xl:col-span-7 space-y-4">
          <h3 className="font-bold text-xs uppercase tracking-wider text-neutral-400 dark:text-neutral-500">
            {t('products.activeCatalog', { count: productsList.length })}
          </h3>
            ...
            {productsList.map(prod => (
              <div key={prod.id} className="p-4 border border-border bg-card rounded-xl flex justify-between items-center hover:border-neutral-300 dark:hover:border-neutral-700 transition-all">
                <div>
                  ...
                  <span className="text-[11px] text-neutral-400 dark:text-neutral-500 font-semibold block mt-0.5">
                    {t('products.catalogPrice', { price: prod.price })}
                  </span>
                </div>
                <div className="text-right">
                  <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-300 border border-neutral-200/40">
                    {t('products.inStock', { stock: prod.stock })}
                  </span>
                </div>
              </div>
            ))}
