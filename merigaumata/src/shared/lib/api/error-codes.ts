...
export const ErrorMessageMap: Record<ErrorCode, string> = {
  ...
  [ErrorCode.CONNECTION_ERROR]: 'Network error. Please check your internet connection.'
};

/**
 * Gets a friendly, localized fallback error message matching the backend error code.
 */
export function getFriendlyErrorMessage(code: string | undefined | null, fallbackMessage?: string): string {
  if (!code) return fallbackMessage || 'An unexpected error occurred.';
  
  // Try exact match first (e.g. '40001')
  let mappedMessage = ErrorMessageMap[code as ErrorCode];
  
  // If no exact match, fallback to generic HTTP status mapping (first 3 digits, e.g. '400')
  if (!mappedMessage && code.length >= 3) {
    const prefix = code.substring(0, 3);
    mappedMessage = ErrorMessageMap[prefix as ErrorCode];
  }

  return mappedMessage || fallbackMessage || `An error occurred (Code: ${code})`;
}
