    ...
    <div className="fixed top-4 inset-x-0 z-50 flex flex-col items-center px-4 w-full">
      <header className="w-full max-w-6xl bg-white dark:bg-[#0a0a0a] border border-neutral-200 dark:border-neutral-800 text-neutral-900 dark:text-neutral-50 px-4 sm:px-6 py-3 rounded-2xl flex items-center justify-between shadow-lg dark:shadow-2xl transition-colors relative z-50">
          ...
          {links.map((link) => {
            ...
            return (
              <Link
                key={link.name}
                href={link.href}
                className={`text-sm font-medium px-4 py-2 rounded-full transition-all relative ${isActive
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          >
            <AppIcon name="cart" className="h-[22px] w-[22px]" strokeWidth={1.5} />
          ...
          >
            {isMenuOpen ? <AppIcon name="close" className="h-[22px] w-[22px]" strokeWidth={1.5} /> : <AppIcon name="menu" className="h-[22px] w-[22px]" strokeWidth={1.5} />}
          </button>
        </div>
      </header>
      ...
      <div
        ref={menuRef}
        className={`lg:hidden w-full max-w-6xl mt-2 overflow-hidden transition-all duration-300 ease-in-out bg-white dark:bg-[#0a0a0a] border-x border-b border-neutral-200 dark:border-neutral-800 rounded-2xl shadow-xl z-40 absolute top-[100%] left-0 right-0 mx-auto transform-origin-top ${isMenuOpen ? 'opacity-100 scale-y-100 translate-y-0' : 'opacity-0 scale-y-0 -translate-y-4 pointer-events-none'
          }`}
          ...
          {links.map((link) => {
            ...
            return (
              <Link
                ...
                className={`text-base font-medium px-4 py-3 rounded-xl transition-all ${isActive
