...
export class TwoFaCodeListener {
  ...
  async handle(payload: TwoFaCodeRequestedPayload): Promise<void> {
    ...
    const { subject, html, text } = await this.templateService.render('2fa-code', payload.locale, {
    ...
    await this.auditRepository.create({
      ...
      eventName: AUTH_EVENTS.TWO_FA_CODE_REQUESTED,
    ...
    try {
      ...
      this.logger.log(
        ...
        'TwoFaCodeListener: 2FA code email sent',
      );
    } catch (err: unknown) {
