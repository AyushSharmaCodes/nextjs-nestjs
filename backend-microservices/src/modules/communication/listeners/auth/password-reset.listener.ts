/**
 * @file communication/listeners/auth/password-reset.listener.ts
 * Handles AUTH_EVENTS.PASSWORD_RESET_REQUESTED — sends the password reset email.
 */

import { Injectable, Logger } from '@nestjs/common';
import { OnEvent } from '@nestjs/event-emitter';
import type { PasswordResetRequestedPayload } from '../../../../shared/events/auth/auth-event-payloads.types';
...
export class PasswordResetListener {
  ...
  async handle(payload: PasswordResetRequestedPayload): Promise<void> {
    ...
    const { subject, html, text } = await this.templateService.render('password-reset', payload.locale, {
      resetUrl: payload.resetUrl,
      resetToken: payload.resetToken,
      expiresAt: payload.expiresAt,
      ipAddress: payload.ipAddress,
      deviceHint: payload.userAgent, // displayed as a hint, not the raw UA string
    });
    ...
    try {
      ...
      this.logger.log({ eventId: payload.eventId, requestId: payload.requestId }, 'PasswordResetListener: email sent');
    } catch (err: unknown) {
      ...
      this.logger.error(
        ...
        `PasswordResetListener: FAILED — ${reason}`,
      );
    }
  }
}
