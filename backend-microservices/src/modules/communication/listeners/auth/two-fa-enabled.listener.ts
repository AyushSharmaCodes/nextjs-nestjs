...
export class TwoFaEnabledListener {
  ...
  async handle(payload: TwoFaEnabledPayload): Promise<void> {
    ...
    await this.auditRepository.create({
      id: auditId, eventId: payload.eventId, eventName: AUTH_EVENTS.TWO_FA_ENABLED,
    ...
    try {
      ...
      this.logger.log({ eventId: payload.eventId, requestId: payload.requestId }, 'TwoFaEnabledListener: confirmation email sent');
    } catch (err: unknown) {
