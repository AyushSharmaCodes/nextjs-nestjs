...
export class AccountLockedListener {
  ...
  async handle(payload: AccountLockedPayload): Promise<void> {
    ...
    const { subject, html, text } = await this.templateService.render('account-locked', payload.locale, {
    ...
    await this.auditRepository.create({
      ...
      eventName: AUTH_EVENTS.ACCOUNT_LOCKED,
    ...
    try {
      ...
      this.logger.log(
        { eventId: payload.eventId, requestId: payload.requestId, reason: payload.reason },
        'AccountLockedListener: lockout email sent',
      );
    } catch (err: unknown) {
