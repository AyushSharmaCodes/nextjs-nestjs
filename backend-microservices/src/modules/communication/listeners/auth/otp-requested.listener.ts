...
export class OtpRequestedListener {
  ...
  async handle(payload: OtpRequestedPayload): Promise<void> {
    ...
    try {
      ...
      this.logger.log({ eventId: payload.eventId, requestId: payload.requestId, purpose: payload.purpose }, 'OtpRequestedListener: OTP email sent');
    } catch (err: unknown) {
      ...
      this.logger.error({
        ...
        eventId: payload.eventId, requestId: payload.requestId, userId: payload.userId, purpose: payload.purpose, auditId, reason,
