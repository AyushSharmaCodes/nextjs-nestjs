...
export class AccountUnlockedListener {
  ...
  async handle(payload: AccountUnlockedPayload): Promise<void> {
    ...
    await this.auditRepository.create({
      id: auditId, eventId: payload.eventId, eventName: AUTH_EVENTS.ACCOUNT_UNLOCKED,
    ...
    try {
      ...
      this.logger.log({ eventId: payload.eventId, requestId: payload.requestId, unlockedBy: payload.unlockedBy }, 'AccountUnlockedListener: unlock email sent');
    } catch (err: unknown) {
