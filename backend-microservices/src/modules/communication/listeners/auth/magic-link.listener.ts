...
export class MagicLinkListener {
  ...
  async handle(payload: MagicLinkRequestedPayload): Promise<void> {
    ...
    const { subject, html, text } = await this.templateService.render(
      'magic-link',
      payload.locale,
      { magicLinkUrl: payload.magicLinkUrl, expiresAt: payload.expiresAt },
    );

    await this.auditRepository.create({
      id: auditId, eventId: payload.eventId, eventName: AUTH_EVENTS.MAGIC_LINK_REQUESTED,
    ...
    try {
      ...
      this.logger.log({ eventId: payload.eventId, requestId: payload.requestId }, 'MagicLinkListener: magic link email sent');
    } catch (err: unknown) {
